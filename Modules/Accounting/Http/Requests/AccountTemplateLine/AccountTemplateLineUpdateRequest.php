...
class AccountTemplateLineUpdateRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $this->validateCyclicReference($validator);
            $this->validateChildrenUpdate($validator);
        });
    }
    ...
    /**
     * اعتبارسنجی عدم وجود ارجاع دوری
     */
    protected function validateCyclicReference($validator): void
    {
        $id = $this->route('id');
        $templateId = $this->input('template_id');
        $parentCode = $this->input('parent_code');
        ...
        // اگر کد حساب تغییر کرده باشد
        if ($accountCode && $parentCode) {
            // بررسی اینکه کد والد زیرمجموعه کد حساب نباشد (ارجاع دوری)
            $children = AccountTemplateLine::where('template_id', $templateId)
                ->where('parent_code', $accountCode)
                ->get();

            foreach ($children as $child) {
                if ($child->account_code === $parentCode) {
                    $validator->errors()->add('parent_code', 'ارجاع دوری شناسایی شد: کد والد نمی‌تواند زیرمجموعه کد حساب باشد');
                    break;
                }
                
                // بررسی عمیق‌تر برای ارجاع دوری
                if ($this->checkCyclicRecursive($templateId, $child->account_code, $parentCode)) {
                    $validator->errors()->add('parent_code', 'ارجاع دوری شناسایی شد');
                    break;
                }
            }
        }
    ...
    /**
     * بررسی بازگشتی ارجاع دوری
     */
    protected function checkCyclicRecursive(int $templateId, string $currentCode, string $targetCode): bool
    {
        $children = AccountTemplateLine::where('template_id', $templateId)
            ->where('parent_code', $currentCode)
            ->get();

        foreach ($children as $child) {
            if ($child->account_code === $targetCode) {
                return true;
            }
            
            if ($this->checkCyclicRecursive($templateId, $child->account_code, $targetCode)) {
                return true;
            }
        }
    ...
    /**
     * اعتبارسنجی بروزرسانی زیرمجموعه‌ها
     */
    protected function validateChildrenUpdate($validator): void
    {
        $id = $this->route('id');
        $templateId = $this->input('template_id');
        $accountCode = $this->input('account_code');
        ...
        // اگر نوع حساب از header به detail تغییر کند
        if ($type === 'detail') {
            $currentRecord = AccountTemplateLine::find($id);
            
            if ($currentRecord && $currentRecord->type === 'header') {
                // بررسی اینکه آیا زیرمجموعه دارد
                $children = AccountTemplateLine::where('template_id', $templateId)
                    ->where('parent_code', $currentRecord->account_code)
                    ->exists();

                if ($children) {
                    $validator->errors()->add('type', 'امکان تغییر نوع حساب از header به detail وجود ندارد زیرا دارای زیرمجموعه است');
                }
            }
        }

        // اگر کد حساب تغییر کند
        if ($accountCode) {
            $currentRecord = AccountTemplateLine::find($id);
            
            if ($currentRecord && $currentRecord->account_code !== $accountCode) {
                ...
                if ($children) {
                    $validator->errors()->add('account_code', 'امکان تغییر کد حساب وجود ندارد زیرا دارای زیرمجموعه است');
                }
            }
        }
    }
